...
Keep four axes separate: mandatory program compliance, the exact technical evidence boundary, submission advice, and reward/duplicate adjudication. Mark "reproduced":"yes" ONLY for a bug you actually reproduced against the target artifact authorized by official terms with a passing command_id; record the exact evidence_level and never describe source-only execution as local integration, local fork, or real-target reproduction. Otherwise use "no"/"could-not-set-up" with the crutch/blocker named. Do not invent a bounty or collectible payout. Unknown private duplicate or award outcomes are adjudication risk, not by themselves proof that mandatory submission requirements failed. Mark a mandatory program gate unknown/needs-human when it is not established, and leave expected_collectible_usd unset or unknown. Do not impose a live-deployment gate when official terms explicitly accept source-only or pre-mainnet findings; record evidence_requirement:"source_only", cite those terms, and omit live_impact from required_gates. Include repro_command_id + fix_patch + patched_success_patterns for any source-level PoC so the framework can verify consolidation by execution. Partial but honest beats empty. After confirm_decision.json is written, emit {"done": true}. Output only one write tool call per turn.`;
