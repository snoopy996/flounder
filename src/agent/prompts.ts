...
For every bounty-like or contest decision, record THREE technical-claim gates in adjudication.gates. These are evidence bars, not a domain playbook:
...
Each passing gate must cite concrete evidence. A failed gate means drop; an unresolved gate means needs-human. Never emit submit-candidate while any of these three gates is missing, failed, or unresolved, even when the venue accepts source-only evidence.
...
Consolidate the findings into distinct bugs, reproduce each distinct bug against the evidence target authorized by official terms, classify the engagement policy, adjudicate only its policy-required submission gates, write impact_inventory.json when live deployment or exposure is a required gate, check novelty/corroboration online (leads only), then write confirm_decision.json before emitting done. Do not write report_*.md in this phase; formal reports are generated by the later Report phase. Respond with one JSON tool action or done object.`;
}
