...
### Choosing a model and configuring access

Follow the [model setup guide](../skills/flounder/reference/models.md) to discover providers, configure OAuth or API keys on the executor, assign a profile, and troubleshoot missing models. It includes DeepSeek and GLM Coding Plan examples and explains what `provider check` does and does not verify.

For live model runs, configure provider credentials on each daemon machine. OAuth-capable providers use `flounder daemon provider login <provider>`; API-key providers can be supplied through the daemon process environment or local secret manager. Check a machine with `flounder daemon provider check <provider>`. For `openai-codex`, an agent can trigger user auth by running `flounder daemon provider login openai-codex`; the command prints a browser URL or device-code instructions for the user to complete, then saves daemon-local auth. If `~/.pi/agent/auth.json` already has the same provider, Flounder imports that provider entry into `~/.flounder/agent/auth.json` on login/check. The server stores provider profiles and routes jobs, but provider credentials stay on the daemon. Do not commit credentials, local environment files, private corpora, or machine-specific paths.

Provider profiles can also register an allowlisted or private model id that is not yet present in pi's generated catalog. In **Settings -> Providers**, type the exact model id; when it is unknown, choose a known model from the same provider as its **Compatibility base**. The base supplies only local transport, tool, context, and reasoning metadata. Flounder still sends the custom id to the provider. The profile contains no credential or endpoint, and its custom definition travels inside each job to the project's selected daemon, so remote executors do not need matching `~/.pi/agent/models.json` files. For example, a separately approved Daybreak Blue profile uses provider `openai-codex`, model `gpt-daybreak-blue-latest`, and compatibility base `gpt-5.6-sol`; the seeded product default is `gpt-5.6-sol`.

Selecting `gpt-6-astra` in the provider editor starts with `medium` thinking; operators can override it before saving. The Astra starter profile is optional and does not replace the Sol product default.

Use **Set default** on a provider card to make it the local control-plane default. This affects new projects, evaluations that choose the runtime default, and ad-hoc API/CLI launches that omit provider/model fields. It does not rewrite existing projects or change the packaged product fallback. **Use product default** clears the local preference and restores `openai-codex · gpt-5.6-sol · xhigh`.
...
2. **Reproduce** each finding against the engagement's **authorized ground truth** — a mainnet fork of the deployed contract and its real verifier, a real released package or local node, or the pinned published source when official source-only terms explicitly make that the evidence boundary. The model writes the reproduction itself. A finding is marked `reproduced` only if it triggers at that exact recorded boundary, using only attacker-real capabilities, with the effect **exhibited** as a concrete observable (a drained balance, a forged output, an accepted invalid input) — never a printed string, never an argument. The evidence level must say which boundary actually ran.
...
5. **Decide**: `confirm_decision.json` + `confirm_report.md`, one row per distinct bug. The product derives four separate outputs from each row: mandatory program compliance, the exact technical evidence boundary, submission advice, and reward/duplicate adjudication risk. Confirm derives `evidence_requirement` and `required_gates` from the engagement's official terms. When those terms require a live deployment or funds-at-risk showing, confirm also writes `impact_inventory.json`; an explicitly source-only or pre-mainnet program can instead accept execution against the pinned published source when its stated scope and reward rules allow that evidence. Unknown private duplicates or award amounts remain explicit uncertainty rather than being mislabeled as failed technical confirmation.
...
A web dashboard to track and drive audits across projects, updating live via SSE. A project is pinned to an **execution daemon** and a **default provider profile**. A provider profile is the model strategy: provider, model, thinking level, an optional custom-model compatibility base, and optional role defaults. Settings can mark one profile as the local runtime default without changing the product fallback or existing projects. The project can also override the provider profile per phase (`prepare`, `map`, `dig`, `confirm`) when one phase needs a different model or reasoning level. The selected daemon must be authenticated for every provider profile the project can use. At launch the control plane includes the selected profiles' custom model definitions in the job claimed by that daemon; credentials remain daemon-local.
...
DAYBREAK_PROFILE=$(curl -s localhost:4500/api/providers \
  -H 'content-type: application/json' \
  -d '{"name":"daybreak-local","provider":"openai-codex","model":"gpt-daybreak-blue-latest","baseModel":"gpt-5.6-sol","thinking":"xhigh"}' | jq -r .id)
curl -X PATCH localhost:4500/api/settings/runtime \
  -H 'content-type: application/json' \
  -d "{\"defaultProviderProfileId\":$DAYBREAK_PROFILE}"             # local default; null restores the product default
PROJECT_UUID=$(curl -s localhost:4500/api/projects \
  -H 'content-type: application/json' \
  -d '{"name":"p","daemonId":1,"dir":"p"}' | jq -r .uuid)       # inherits the local default
