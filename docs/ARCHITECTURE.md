...
The default audit provider is `openai-codex` (`gpt-5.5`). The continuous session requires daemon-local provider auth configured through `flounder daemon provider login openai-codex` or daemon environment variables; it does not reuse the standalone codex CLI's credentials, so an unauthenticated run fails fast with an actionable Flounder command (use `--mock-llm` for offline checks). Per project constraint, the session driver targets pi-mono providers such as `openai-codex`; `claude-code` is not used as a session backend (it is not permitted outside Claude apps and needs no API key here).
...
- `flounder_run`: with a clue, the full prepare -> sealed map/dig -> confirm pipeline; with source paths, the sealed map/dig source audit.
...
**UI (`src/server/ui`).** A React/Vite TypeScript app compiled into `dist/server/public` and served by the Node control plane. It is **one client of the API above** — it can be ignored entirely in favor of the API. A project is created by selecting an execution daemon, a default provider profile, source/build/corpus paths, coverage, and optional phase provider overrides. Its detail view shows the **prepare → map → dig → confirm** workflow, the current phase, elapsed timing, live model activity, and scope coverage. Below it: a scope queue you can hand-order (`Top`) and skip/resume; findings that **stream in as each scope lands** and update status through refutation, each with a per-finding **Confirm** button and its real-target outcome; a project-wide **Confirm pending on real target**; per-scope and per-finding durations; and viewable Markdown **reports**. The cross-project **Findings** view tracks every finding plus submission status. **Settings** holds provider profiles (provider + model + thinking, with optional phase/role defaults) and daemon CRUD. UI state derivation lives in `src/server/ui/src/domain.ts`, API types/client code live in `src/server/ui/src/api.ts`, and every action remains a single REST call.
