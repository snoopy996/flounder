...
The execution backend is explicit. `AuditorConfig.sandboxBackend` defaults to `auto`, which uses the OCI image `flounder-sandbox:latest` when it is available and otherwise returns a policy failure instead of silently running model-generated code on the host. `--sandbox-backend oci` requires a container. `--sandbox-backend host --allow-host-execution` is a trusted-local escape hatch for deterministic fixtures and development smoke tests; it preserves the isolated `HOME`, `TMPDIR`, and package-cache environment, but it is not treated as kernel isolation. The OCI runner bind-mounts only the copied workspace (and the optional persistent package cache), disables image pulling at execution time, drops Linux capabilities, sets `no-new-privileges`, uses a read-only root filesystem with tmpfs temp dirs, applies pids/memory/CPU limits when configured, and uses Docker's `--network none` for sealed inspect/confirm commands.
...
Model and provider selection stays runtime-configured. Do not assume every model family is available through every provider. In the dashboard a **provider profile is the vendor only** (its credentials live on the daemon via pi `/login`); the **model and thinking level are chosen per phase** (prepare / map / dig / confirm) in each project's config, from the vendor's live model list (`config.phases`). `launchSpec` resolves the verb's primary phase into the run's top-level model/thinking and maps map/dig (refute←dig) into per-role overrides — so different phases can run different models.
...
**REST API + self-describing catalog (`src/server/app.ts`).** A `node:http` server that binds to `127.0.0.1` by default. Every workflow operation is a REST resource so an AI agent can drive the whole flow without the UI: **project** (CRUD), **run** (`POST /api/projects/:name/runs` **enqueues a job** and returns its `jobId`; `GET /api/runs/:id`; `POST /api/runs/:id/stop`; `GET /api/runs/:id/artifact?name=` serves an allowlisted report file), and read-only **scope** / **finding** (paginated + filterable) / **confirm-decision**. Findings carry two operator axes: `PATCH /api/projects/:name/scopes/:id` with `{prioritize:true}` bumps a scope to the front of the dig queue, and `PATCH /api/findings/:id/tracking` advances a finding's submission state. **Confirm is finding-grained**: `POST …/runs` with `verb:"confirm"` and *no* run dir reproduces every still-pending confirmable finding (the control plane locates each one's source run dir from its `run_id` and passes the pending set); with `findingId` it confirms just that one; a re-run skips already-decided findings (resume by `confirm_status`). A cross-project **`GET /api/bugs`** returns every finding (joined with project) plus status/tracking aggregates, and **`/api/daemons`** is full CRUD (mint a token in-UI, rename, revoke). `GET /api` returns a catalog of every endpoint (method, path, params, body, summary) — an agent fetches it once to learn the surface. The execution-plane protocol — `/api/daemon/*` (register, SSE stream, claim, run-start, progress PATCH, activity, job-status), each **bearer-token-authenticated** — is **hidden from the catalog** (it is machine-to-machine, not agent-facing). Routing is a data-driven table, so the catalog and the handlers cannot drift.
...
**UI (`src/server/public/index.html`).** A zero-dependency vanilla SPA (clean light-default theme + dark toggle). It is **one client of the API above** — it can be ignored entirely in favor of the API. A project's detail is a **prepare → map → dig → confirm** pipeline (GitHub-Actions-style): the view auto-follows the running phase, each node shows its elapsed time, and the scope being dug is marked `auditing`. Below it: a scope queue you can hand-order (`↑ Top`) and skip/resume; findings that **stream in as each scope lands** and update status through refutation, each with a per-finding **Confirm** button and its real-target outcome; a project-wide **Confirm pending on real target**; per-scope and per-finding durations; and viewable Markdown **reports**. A separate **Bugs** tab is a cross-project board (every finding + submission tracking), and **Settings** holds vendor-only provider profiles (model & thinking are picked **per phase** in each project's config, from the vendor's live model list) and **daemon** CRUD. Every action is a single API call.
