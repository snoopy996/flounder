...
- **Governed harness experiments.** `flounder experiment` mines verifier-grounded baseline failures, records passing behavior to preserve, constrains proposals to an explicit harness-file allowlist, and compares paired baseline/candidate Evaluations. The deterministic gate can recommend promote, reject, or more samples, but the evaluator, expected answers, safety policy, merge, and deployment authority remain outside the loop.
...
| Harness experiments | Clusters persisted Evaluation failures, exports bounded candidate briefs, and applies a paired no-regression promotion gate without granting code-merge or release authority. |
...
The Evaluations view creates and operates durable audit, benchmark, regression, and verification run groups. Its Harness mode turns finished groups into governed baseline/candidate experiments with mined failure patterns, bounded proposal refinement, compact metric comparison, and an explicit promotion decision. Each item uses a hidden tracking project so target-name collisions cannot mix benchmark runs into operator Projects. Project jobs are claimed ahead of queued Evaluation jobs on a shared daemon; already-running work is never preempted. Lifecycle completion is never presented as an evidence pass by itself, and a promotion recommendation never merges or deploys code.
